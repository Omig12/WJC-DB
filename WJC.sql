...
create Table Transfer (reserveID int references Population(reserveID) on update cascade, speciesID int references Population(speciesID) on update cascade, Amount int references Population(population), destinationID int references Reserve(reserveID), transfer_Date timestamp, primary key (reserveID, destinationID, Amount, speciesID, transfer_date));
...
/* Triggers */
delimiter //
CREATE DEFINER=`root`@`localhost` TRIGGER `Transfer_BINS` BEFORE INSERT ON `Transfer` 
FOR EACH ROW
BEGIN	
...
delimiter //
CREATE DEFINER=`root`@`localhost` TRIGGER `Transfer_AINS` AFTER INSERT ON `Transfer` 
FOR EACH ROW
BEGIN	
	IF NEW.Amount > 0 
	THEN
...
select * from (((Animal natural join Population) natural join Reserve) natural join Hazard) natural join Animal as A;
